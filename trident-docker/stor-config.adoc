...
`300`
 ...
 a| 
`sanType`
| *Supportato solo per `ontap-san` il driver.* Utilizzare per selezionare `iscsi` iSCSI, `nvme` NVMe/TCP o `fcp` SCSI over Fibre Channel (FC). *'fcp' (SCSI over FC) è una funzionalità di anteprima tecnica nella versione Trident 24,10.* | `iscsi` se vuoto 
