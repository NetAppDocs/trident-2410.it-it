...
Questa `showmount` funzionalità consente a un client NFS di richiedere all'SVM un elenco di esportazioni NFS disponibili. Un pod sul cluster Kubernetes può impartire un `showmount -e` comando sulla LIF dati e ricevere un elenco di mount disponibili, compresi quelli ai quali non ha accesso. Sebbene questo, di per sé, non sia un compromesso in termini di sicurezza, fornisce informazioni non necessarie che potrebbero aiutare un utente non autorizzato a connettersi a un'esportazione NFS.
